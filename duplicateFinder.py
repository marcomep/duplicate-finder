    ...
    for full_name in file_list:
        ...
        if os.path.isfile(full_name):
            ...
            if sha_file in not_duplicate:
                ...
                if (not base_in_dir.endswith('/')):
                    base_dir += "/"

                file_name = os.path.basename(full_name)
                len_base_in_dir = len(base_in_dir)
                subpath = file_name[len_base_in_dir:len_base_in_dir - len(file_name)]
                out_file_clone = os.path.join(input_cfg.output, subpath, "DELETE_" + file_name)
                shutil.copyfile(full_name, out_file_clone)

                file_name = os.path.basename(not_duplicate[sha_file]["path"])
                len_base_in_dir = len(base_in_dir)
                subpath = not_duplicate[sha_file]["path"][len_base_in_dir:len_base_in_dir - len(file_name)]
                out_file_original = os.path.join(input_cfg.output, subpath, "ORIGINAL_" + file_name)
